...
$name = $altText ? Str::slug($altText) . '-' . uniqid() : md5(time() . rand(10, 1000));
   ...
   if ($isColumn) {
    ...
    public static function getProductHtml($product)
    {
        ...
        $html .= '<div class="position-absolute end-0 top-0 z-1 d-flex flex-column align-items-end p-2">';

        // Temperature Sensitive Badge
        if ($product->temp_sensitive) {
            $html .= '
        <div class="temp-sensitive-badge mb-1">
            <img src="' . asset('frontend/img/temp-sensitive.png') . '"
        ...
        // Sale Badge
        if ($product->is_sale) {
            $html .= '<span class="badge bg-success mb-1">Sale</span>';
        }

        // New Badge
        if ($product->is_new) {
            $html .= '<span class="badge bg-info mb-1">New</span>';
        }

        // Hot Badge
        if ($product->is_hot) {
            $html .= '<span class="badge bg-warning text-dark mb-1">Hot</span>';
        }

        // Best Sell Badge
        if ($product->is_best_sell) {
            $html .= '<span class="badge bg-danger">Best Sell</span>';
        }
...
<div class="mb-3">
    ...
    {
        $products = Product::select('products.slug', 'products.color_id', 'colors.name', 'colors.code')
