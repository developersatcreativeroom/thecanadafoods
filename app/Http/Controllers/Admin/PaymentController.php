...
{   
    ...
    public function list(Request $request){ 
        $page = $request->page;
        $rows = Payment::select('payments.*','users.first_name','users.last_name','users.email','users.phone', 'orders.order_no')
        ->leftjoin('users','users.id','=','payments.user_id')
        ->join('orders','orders.id','=','payments.order_id')
        ->latest('payments.id')->paginate($this->pagerecords, ['*'], 'page', $page);
    ...
    public function view($id){
        ...
        if($row == null){
            return to_route('admin.payments');
        } 
        if($row->user_id == null){
