                ...
                <div class="breadcrumb-content">
                    ...
                    <nav>
                        <ol class="breadcrumb justify-content-center">
                            ...
                            <li class="breadcrumb-item fw-bold" aria-current="page"><a href="{{route('category',$productCategory->slug)}}">{{$productCategory->name}}</a></li>
                            <li class="breadcrumb-item fw-bold" aria-current="page">{{$product->title_h1 ? $product->title_h1 : $product->name ;}}</li>
                        </ol>
                    </nav>
                </div>
                                    ...
                                    <div class="quickview-product-slider swiper">
                                        <div class="swiper-wrapper">
                                            <div class="swiper-slide text-center">
                                                <img src="{{ asset('storage/products/') }}/{{$product->id}}/{{$product->image}}" alt="{{$product->name}}" class="img-fluid">
                                            </div> 
                                            ...
                                            @if(!empty($product) && ($product->images != null || $product->images != '') && count($product->images) > 0)
                                                ...
                                                <div class="swiper-slide text-center">
                                                    <img src="{{ asset('storage/products/') }}/{{$product->id}}/gallery/{{$image->image}}"  class="img-fluid">
                                                </div>
                                                @endforeach
                                            @endif
                                    ...
                                    <div class="product-thumbnail-slider swiper mt-10">
                                        <div class="swiper-wrapper">
                                            <div class="swiper-slide product-thumb-single rounded-2 d-flex align-items-center justify-content-center">
                                                <img src="{{ asset('storage/products/') }}/{{$product->id}}/{{$product->image}}" alt="{{$product->name}}" alt="{{$product->name}}" class="img-fluid">
                                            </div>
                                            @if(!empty($product) && ($product->images != null || $product->images != '') && count($product->images) > 0)
                                                @foreach($product->images as $image)
                                                    <div class="swiper-slide product-thumb-single rounded-2 d-flex align-items-center justify-content-center">
                                                        <img src="{{ asset('storage/products/') }}/{{$product->id}}/gallery/{{$image->image}}" alt="" class="img-fluid">
                                                    </div>
                                                @endforeach
                                            @endif 
                                        </div>
                                ...
                                <div class="product-info detail-info">
                                    ...
                                    @if($product->total_rating > 0) 
                                        ...
                                        <div class="d-flex align-items-center flex-nowrap star-rating fs-xxs mb-2">
                                            ...
                                            <span class="flex-shrink-0">({{$product->total_rating}} {{$product->total_rating > 1 ? 'ratings' : 'rating'}})</span>
                                        </div>
                                    @else 
                                    ...
                                    <div class="pricing mt-2">
                                        ...
                                        @endif
                                        <span class="fw-bold fs-xs deleted ms-1 old-price">{{$product->getOldPrice()}}</span>
                                    ...
                                    @if(!$product->is_variant)
                                        <div class="d-flex mb-2 mt-3 align-items-center">
                                            ...
                                            {!!$product->stock > 0 ? '<p class="mb-0 text-success">In Stock</p>' : '<p class="mb-0 text-danger">Out of Stock</p>'!!}
                                    ...
                                    @if($product->tags != null) 
                                        <div class="tt-category-tag align-items-center">
                                            ...
                                            @foreach(json_decode($product->tags) as $tag)
                                                <a href="#" class="text-muted fs-xxs" rel="tag">{{$tag}}</a>
                                            @endforeach
                                        </div>
                                    @endif
                                    ...
                                    @if(isset($itemattributes) && count($itemattributes) > 0)
                                        <div class="attributes">
                                            @foreach($itemattributes as $itemattribute)
                                                ...
                                                <h6 class="fs-md mb-2 mt-3">{{$itemattribute['attribute_name']}}:</h6>

                                                <ul class="product-radio-btn d-flex align-items-center gap-2 attribute-select" data-attribute="{{$itemattribute['attribute_id']}}">
                                                    @foreach($itemattribute['options'] as $option)
                                                        <li><a class="attribute-option" data-id="{{$option['attribute_option_id']}}" href="#"><label>{{$option['attribute_option_name']}}</label></a></li>
                                                    @endforeach
                                                </ul>
                                    ...
                                    <div class="d-flex align-items-center gap-4 flex-wrap">
                                        ...
                                        <div class="product-qty d-flex align-items-center detail-qty">
                                            ...
                                            <input type="text" class="qty-val product-qty-val only-numbers" value="1" >
                                            <button class="change-qty qty-up" data-stock="@if(!$product->is_variant){{$product->stock}}@endif">+</button>
                                        </div>


                                        @php
                                            $disabled = ''; if($product->is_variant){ $disabled = 'disabled'; } 
                                        @endphp

                                        @php
                                            $stringAdd = $isEnquiryWebsite['is_enquiry_website'] ? 'Add to enquiry' : 'Add to cart' ;
                                            $stringRemove = $isEnquiryWebsite['is_enquiry_website'] ? 'Remove from enquiry' : 'Remove from Cart' ;
                                        @endphp
                                        ...
                                        @if($already && !$product->is_variant)
                                            ...
                                            <a href="#" class="btn btn-secondary btn-md text-white add-cart {{$disabled}}" {{$disabled}} data-key="{{$product->slug}}" data-page="detail">
                                                <span class="me-2"><i class="fa-solid fa-bag-shopping"></i></span>
                                                {{$stringAdd}}
                                            </a>
                                        ...
                                        @if($alreadyWishlist)
                                            <a {{$disabled}} class="btn btn-md add-wishlist active {{$disabled}}" data-key="{{$product->slug}}"><i class="fa-regular fa-heart"></i></a>
                                        @else
                                            <a {{$disabled}} class="btn btn-md add-wishlist {{$disabled}}" data-key="{{$product->slug}}"><i class="fa-regular fa-heart"></i></a>
                                        @endif
                                                ...
                                                <div class="detail-extralink services-cont">
                                                    ...
                                                    <div class="row">
                                                        @foreach($product->services as $service)
                                                            @php
                                                                $alreadyService = App\Helper::serviceAlreadyInCart($product->id,null,$service);
                                                                
                                                            @endphp

                                                            <div class="col-md-12 col-lg-12">
                                                                <div class="hero-card box-shadow-outer-6 wow fadeIn animated mb-10 hover-up d-flex p-3">
                                                                    ...
                                                                    <div class="pl-10 align-self-center">
                                                                        @if(!$product->is_variant)
                                                                            @if($alreadyService)
                                                                                <button class="button btn-sm p-2 service remove-service" data-key="{{$product->slug}}" data-service="{{$service->slug}}" @if(!$already && !$product->is_variant) disabled @endif>Remove Service</button>
                                                                            @else
                                                                                <button class="button btn-sm p-2 service add-service" data-key="{{$product->slug}}" data-service="{{$service->slug}}" @if(!$already && !$product->is_variant) disabled @endif>Add Service</button>
                                                                            @endif
                                                                        @else
                                                                            <button class="button btn-sm p-2 service add-service" data-key="{{$product->slug}}" data-service="{{$service->slug}}" disabled >Add Service</button>
                                                                        @endif
                                                                    </div>
                                                                </div>
                                                            </div>
                                    ...
                                    <tbody>
                                        ...
                                        @if(count($product->specifications) > 0)
                                            ...
                                            <tr class="stand-up">
                                                ...
                                                <td>
                                                    <p>{{$specification['value']}} @if($specification['units']) {{$specification['units']}} @endif</p>
                                                </td>
                                            </tr>
                                            @endforeach
                                        @endif
                            ...
                            <div class="d-flex flex-wrap align-items-center justify-content-between">
                                <div class="top-left">
                                    ...
                                    <div class="d-flex align-items-center">
                                        <span class="ti text-warning" style="--rating:{{$product->average_rating}}"></span>
                                        <span class="ms-2 text-dark">({{$product->average_rating}} out of 5)</span>
                                    </div> 
                                </div>
                                <div class="jdgm-histogram jdgm-temp-hidden">
                                    @foreach($product->rating_distribution as $stars => $data)
                                    <div class="jdgm-histogram__row filter_rating" data-rating="{{$stars}}" data-frequency="{{$data['count']}}" data-percentage="{{$data['percentage']}}">
                                        <div class="jdgm-histogram__star" role="button" aria-label="{{$data['percentage']}}% ({{$data['count']}}) reviews with 5 star rating" tabindex="0">
                                            <div class="d-flex align-items-center justify-content-between">
                                                <span class="ti text-warning" style="--rating:{{$stars}}"></span>
                                            </div>
                                        </div>
                                        <div class="jdgm-histogram__bar">
                                            <div class="jdgm-histogram__bar-content" style="width: {{$data['percentage']}}%;"> </div>
                                        </div>
                                        <div class="jdgm-histogram__percentage">{{$data['percentage']}}%</div>
                                        <div class="jdgm-histogram__frequency">({{$data['count']}})</div>
                                    </div>
                                    @endforeach
                                  
                                    <div class="jdgm-histogram__row jdgm-histogram__clear-filter filter_rating" data-rating="null" tabindex="0">See all reviews</div>
                                </div>
                            ...
                            @foreach($similarProducts as $similarProduct)
                                <div class="horizontal-product-card card-md d-sm-flex align-items-center bg-white rounded-2 gap-3 mt-4">
                                    <div class="thumbnail position-relative rounded-2">
                                        <a href="#"><img src="{{ asset('storage/products/') }}/{{$similarProduct->id}}/{{$similarProduct->image}}" alt="product" class="img-fluid"></a>
                                        <div class="product-overlay position-absolute start-0 top-0 w-100 h-100 d-flex align-items-center justify-content-center gap-2 rounded-2">
                                            <a href="{{route('product',$similarProduct->slug)}}" class="rounded-btn"><i class="fa-solid fa-eye"></i></a>
                                        </div>
                                    </div>
                                        ...
                                        <div class="d-flex align-items-center flex-nowrap star-rating mt-1">
                                            ...
                                            @if($reviewShow && $similarProduct->total_rating > 0) 
                                                ...
                                                <div class="rating-result" >
                                                    <div class="d-flex align-items-center flex-nowrap star-rating fs-xxs mb-2">
                                                        <span class="badge text-bg-secondary text-white verified-badge"> <a href="#reviews_section" class="text-white"><i class="fas fa-star"></i> {{$similarProduct->average_rating}}</a></span> &nbsp;
                                                    </div>
                                                </div>
                                            @endif
                                        </div>
                                        <a href="{{route('product',$similarProduct->slug)}}" class="Minimum Cart Amount d-block fs-sm fw-bold text-heading title d-block">{{$similarProduct->name}}</a>
                                        <div class="pricing mt-0">
                                            <span class="fw-bold text-secondary text-price">{{$similarProduct->getPrice()}}</span>
                                        </div>
...
@php
    ...
    $breadcrumbs = [
        ...
        ['name' => $product->title_h1 ? $product->title_h1 : $product->name , 'url' => route('product', $product->slug)],
    ];
