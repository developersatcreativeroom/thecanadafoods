            ...
            <div class="card">
			  <div class="card-header d-flex justify-content-between p-0">
                <h3 class="card-title p-3">Order details</h3>
				        <div class="ml-auto py-2 px-3"><a class="btn btn-primary" href="{{route('admin.orders')}}">List</a></div>
              </div>
              <!-- /.card-header -->
              <!-- form start -->
                ...
                <div class="card-body">
					<div class="row">
						...
						<div class="row">
							...
							<div class="col-sm-9">
							<p class="form-control-plaintext">{{$row->order_no}}</p>
							</div>
						</div>
						...
						<div class="row">
							...
							<div class="col-sm-9">
							<p class="form-control-plaintext order-status">{{$row->order_status}}</p>
							</div>
						</div>
						...
						@if($row->order_notes)
						<div class="row">
							<label class="col-sm-3 col-form-label">Order notes</label>
							<div class="col-sm-9">
						...
						<div class="row">
							...
							<div class="col-sm-9">
							<p class="form-control-plaintext">{{$row->billing->address_line_1}} {{$row->billing->address_line_2}} {{$row->billing->street}}, {{$row->billing->city}}<br> {{$row->billing->state}}-{{$row->billing->postal}}, {{$row->billing->country}}</p>
							</div>
						</div>
						...
						<h5>User Details</h5>
							...
							<div class="row mb-3">
								<label class="col-sm-3 col-form-label">Order Status <span class="text-danger">*</span></label>
								<div class="col-sm-9">
									<select class="form-control {{ $errors->has('status') ? ' is-invalid' : '' }}" id="status">
										<option value="">--Select--</option>
										@foreach(config('constants.ORDER_STATUSES') as $key => $status)
							...
							<div class="row mb-3">
								...
								<div class="col-sm-9">
									<textarea class="form-control {{ $errors->has('note') ? ' is-invalid' : '' }}" id="note" rows="2" placeholder="Enter Note (Optional)" name="note">@if(old('note')!=null){{old('note')}}@endif</textarea>
								</div>
								@if($errors->has('note'))
									<span class="invalid-feedback">
										{{ $errors->first('note') }}
							...
							<div class="row">
								...
								<div class="col-sm-9">
								<button type="submit" class="btn btn-primary" id="update-status">Update</button>
								</div>
							</div>
							...
							<div id="order-history">
								@include('admin.order.view-history')
							</div>
										...
										<table class="table table-bordered">
											...
											<tbody>
												...
												<tr>
													...
													<td>
														@if($product->old_price)
														{{$row->currency}}{{App\Helper::numberFormat(($product->old_price-$product->price)*$product->quantity)}}
														@endif
													</td>
													<td>{{$product->sku}}</td>
													@if($row->payment)
