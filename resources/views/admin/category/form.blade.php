                        ...
                        <div class="card">
                            <div class="card-header d-flex justify-content-between p-0">
                                ...
                                <div class="ml-auto py-2 px-3"><a class="btn btn-primary"
                                        href="{{ route('admin.categories') }}">List</a></div>
                            </div>
                            <!-- /.card-header -->
                            <!-- form start -->
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-md-6">
                                            ...
                                            <div class="form-group">
                                                <label for="parent_category_id">Parent Category <span
                                                        class="text-danger">*</span></label>
                                                <select
                                                    class="form-control {{ $errors->has('parent_category_id') ? ' is-invalid' : '' }}"
                                                    id="parent_category_id" name="parent_category_id">
                                                    <option value="">--Select Value--</option>
                                                    @foreach ($categories as $categorySingle)
                                                        <option @if (!empty($row) && $row->id == $categorySingle['id']) disabled @endif
                                                            @if (old('parent_category_id') != null && old('parent_category_id') == $categorySingle['id']) selected @elseif(!empty($row) && $row->parent_category_id == $categorySingle['id']) selected @endif
                                                            value="{{ $categorySingle['id'] }}">
                                                            {{ $categorySingle['name'] }}</option>

                                                        @foreach ($categorySingle['children'] as $children1)
                                                            <option @if (!empty($row) && $row->id == $children1['id']) disabled @endif
                                                                @if (old('parent_category_id') != null && old('parent_category_id') == $children1['id']) selected @elseif(!empty($row) && $row->parent_category_id == $children1['id']) selected @endif
                                                                value="{{ $children1['id'] }}">&nbsp;&nbsp; &#9679;
                                                                {{ $children1['name'] }}</option>

                                                            @foreach ($children1['children'] as $children2)
                                                                <option disabled
                                                                    @if (old('parent_category_id') != null && old('parent_category_id') == $children2['id']) selected @elseif(!empty($row) && $row->parent_category_id == $children2['id']) selected @endif
                                                                    value="{{ $children2['id'] }}">&nbsp;&nbsp;&nbsp;&nbsp;
                                            ...
                                            <div class="form-group">
                                                ...
                                                <textarea class="form-control {{ $errors->has('short_description') ? ' is-invalid' : '' }}" id="short_description"
                                                    rows="4" placeholder="Enter Short description" name="short_description">
@if (old('short_description') != null)
{{ old('short_description') }}
@elseif(!empty($row->short_description))
...
@endif
                                            ...
                                            <div class="form-group">
                                                <label for="image">Category Image <span
                                                        class="text-danger">*</span></label>
                                                <input type="file" accept="image/png, image/gif, image/jpeg, image/webp"
                                                    class="form-control dropify {{ $errors->has('image') ? ' is-invalid' : '' }}"
                                                    data-default-file="{{ !empty($row) && ($row->image != null || $row->image != '') ? asset('storage/categories/') . '/' . $row->id . '/' . $row->image : '' }}"
                                                    id="image" name="image">
                                                @if ($errors->has('image'))
    ...
    <select
        ...
        @for($i = 1; $i <= $maxPriority; $i++)
            ...
            <option value="{{ $i }}"
                {{ old('priority', isset($row) ? $row->priority : '') == $i ? 'selected' : '' }}>
